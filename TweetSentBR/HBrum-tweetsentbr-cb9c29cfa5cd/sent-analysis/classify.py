...
def load_data(file_name, shuffle=True, rmttsbr=False, verbose=False):
	global CLASSES
	file_sentences = []
	file_labels = []
	file_ids = []
...
def run(classifier='linearsvm', options='bow negation emoticon emoji senti_words postag', verbose=False, train_file='data/corpus/trainTT',
	    test_file='data/corpus/testTT', fs=False, fold=True, nb_classes=3, md_options='', embedding_file=False, rmttsbr=False):
	# PARAMS
	fold = fold
	fs = fs
	...
	# LOAD DATA
	train_data = load_data(train_file, rmttsbr=rmttsbr, shuffle=True, verbose=verbose)
...
if __name__ == '__main__':
	...
	run(classifier=classifier, md_options=md_options, fs=fs,nb_classes=nb_classes, fold=fold, options=options, verbose=verbose,
		train_file=train_file, test_file=test_file, embedding_file=embedding_file, rmttsbr=rmttsbr)
	#new_run(sys.argv[1:])
